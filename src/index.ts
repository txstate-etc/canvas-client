...
class CanvasConnector {
  ...
  constructor (token: string, canvasUrl: string|undefined = process.env.CANVAS_BASE_URL) {
    if (!canvasUrl) throw new Error('Tried to create a Canvas client without a base URL.')
...
const connectors = (process.env.CANVAS_TOKENS || '').split(',').map(token => new CanvasConnector(token))
function getConnector () {
  return connectors.reduce((leastBusyConnector, connector) => connector.tasks() < leastBusyConnector.tasks() ? connector : leastBusyConnector, connectors[0])
}

class CanvasAPI {
  async get (url: string, params: any = {}): Promise<any> {
    return getConnector().get(url, params)
  }

  async getall (url: string, params: any = {}): Promise<any[]> {
    return getConnector().getall(url, params)
  }

  async delete (url: string, params: any = {}): Promise<any> {
    return getConnector().delete(url, params)
  }

  async put (url: string, payload: any): Promise<any> {
    return getConnector().put(url, payload)
  }

  async post (url: string, payload: any): Promise<any> {
    return getConnector().post(url, payload)
  }
